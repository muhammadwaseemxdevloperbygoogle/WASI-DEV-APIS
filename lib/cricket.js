        ...
        */
        ...
        if (!players.batting || players.batting.length === 0) {
            try {
                ...
                // Parse Bowling (New UI Grid)
                // Class matching "scorecard-bowl-grid"
                $sc('.scorecard-bowl-grid').each((i, row) => {
                    // Check if it's a header or player row
                    // Header usually has "Bowler" text
                    if ($(row).text().includes("Bowler")) return;

                    const cols = $(row).children();
                    if (cols.length >= 4) {
                        const name = clean($(cols).first().text()); // 1st col Name
                        const overs = clean($(cols).eq(1).text());  // 2nd col Overs
                        const maidens = clean($(cols).eq(2).text()); // 3rd col Maidens
                        const runs = clean($(cols).eq(3).text());    // 4th col Runs
                        const wickets = clean($(cols).eq(4).text()); // 5th col Wickets
                        // 6th NB, 7th WD, 8th ECO

                        if (name && overs && wickets) {
                            // Check duplicate
                            if (!players.bowling.find(p => p.name === name)) {
                                players.bowling.push({ name, overs, maidens, runs, wickets });
                            }
                        }
                    }
                });

                // Fallback for older table structure (if grid not found)
                if (players.bowling.length === 0) {
                    $sc('.cb-scrd-itms').each((i, row) => {
                        const txt = $(row).text();
                        if (txt.includes('.')) {
                            const name = clean($(row).find('.cb-col-40, .cb-col-27').first().text());
                            const overs = clean($(row).find('.cb-col-8, .cb-col-10').first().text());
                            const wickets = clean($(row).find('.cb-col-8.text-right.text-bold').text());
                            const runs = clean($(row).find('.cb-col-10').eq(1).text());

                            if (name && overs && wickets && !players.bowling.find(p => p.name === name)) {
                                players.bowling.push({ name, overs, wickets, runs });
                            }
                        }
                    });
                }
