                ...
                if (apiData) {
                    ...
                    if (apiData.miniscore) {
                        ...
                        if (bat && bat.teamScore) {
                            ...
                            // Batting team exists but presumably 0/0
                            score = `${bat.teamId} yet to bat`;
                        }

                        const ms = apiData.miniscore;
                        // Better Team Names from Miniscore
                        const teamBatName = ms.batTeam ? (ms.batTeam.teamName || ms.batTeam.teamSName) : 'Team A';
                        const teamBowlName = ms.bowlTeam ? (ms.bowlTeam.teamName || ms.bowlTeam.teamSName) : 'Team B';

                        players = {
                            teamBat: teamBatName,
                            teamBowl: teamBowlName,
                            batting: [],
                            bowling: []
                        };
