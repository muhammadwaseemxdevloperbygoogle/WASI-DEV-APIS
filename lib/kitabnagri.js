 ...
 */
async function searchBooks(query, isRetry = false) {
    try {
        ...
        // If no good matches and not already a retry, try fallbacks
        if (!isRetry && (results.length === 0 || results[0].similarity < 0.3)) {
            // Try common phonetic fixes first (faster than searching first word)
            ...
            if (fixedQuery !== query) {
                const fixedRes = await searchBooks(fixedQuery, true);
                if (fixedRes.status && fixedRes.results.length > 0) return fixedRes;
            }

            // Fallback to first word if query is multi-word
            const words = query.split(' ');
            if (words.length > 1) {
                const firstWord = words[0];
                if (firstWord.length > 3) {
                    const retryRes = await searchBooks(firstWord, true);
                    if (retryRes.status) {
                        const broader = retryRes.results.map(r => ({
                            ...r,
                            similarity: getSimilarity(r.title, query)
                        })).sort((a, b) => b.similarity - a.similarity);
                        return { status: true, results: broader };
                    }
                }
            }
        }
